...
- `coerceString` (`String()` coercion), `fromStringRaw`, `stringAt`, `localeCompare`
- Non-locale casing: `asciiLowerCase`, `asciiUpperCase`
- `Intl` operators: `intlCollatorCompare`, `intlSegment`, `intlListFormat`
...
- **BREAKING**: Removed `padLeft`/`padRight` and `trimLeft`/`trimRight` aliases; use `padStart`/`padEnd` and `trimStart`/`trimEnd`
