...
describe('toLowerCase', () => {
  ...
  it(
    'should take a string and convert to lower case with locale array',
    marbles((m) => {
      const input = m.hot('-a-b-c-|', { a: 'HËLLO', b: 'TëstinG', c: 'fOOBÄr' });
      const subs = '^------!';
      const expected = m.cold('-x-y-z-|', { x: 'hëllo', y: 'tësting', z: 'foobär' });
      m.expect(input.pipe(toLowerCase(['de-DE', 'en-EN']))).toBeObservable(expected);
      m.expect(input).toHaveSubscriptions(subs);
    }),
  );

  it(
    'should take a string and convert to lower case with Observable locale',
    marbles((m) => {
      const input = m.hot('-a-b-c-|', { a: 'HËLLO', b: 'TëstinG', c: 'fOOBÄr' });
      const subs = '^------!';
      const expected = m.cold('-x-y-z-|', { x: 'hëllo', y: 'tësting', z: 'foobär' });
      m.expect(input.pipe(toLowerCase(of('de-DE')))).toBeObservable(expected);
      m.expect(input).toHaveSubscriptions(subs);
    }),
  );

  it(
    'should take a string and convert to lower case with Observable locale array',
    marbles((m) => {
      const input = m.hot('-a-b-c-|', { a: 'HËLLO', b: 'TëstinG', c: 'fOOBÄr' });
      const subs = '^------!';
      const expected = m.cold('-x-y-z-|', { x: 'hëllo', y: 'tësting', z: 'foobär' });
      m.expect(input.pipe(toLowerCase(of(['de-DE', 'en-EN'])))).toBeObservable(expected);
      m.expect(input).toHaveSubscriptions(subs);
    }),
  );
});
